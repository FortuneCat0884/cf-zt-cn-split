...
def update_split_tunnels(cidrs, domains):
    ...
    # 动态分配配额：域名取 TARGET_DOMAIN_N 条，剩余配额给 IP
    max_domains = min(TARGET_DOMAIN_N, len(domains))

    # AI 域名占用一部分规则额度
    max_ips = min(MAX_RULES - len(ai_entries) - max_domains, len(cidrs))

    # 域名规则在前，IP 规则在后
    domain_entries = [
        {
            "host": d,
            "description": "CN Domain"
        }
    ...
    # AI + CN Domain + CN IP
    routes = ai_entries + domain_entries + ip_entries
